...
import { MapPin, Thermometer, Droplets, AlertCircle, Truck, Navigation, Route, RefreshCw } from 'lucide-react';
...
export const LogisticsView: React.FC<LogisticsViewProps> = ({ complianceMode }) => {
    ...
    const [optimizationData, setOptimizationData] = React.useState<{ savings: string, routes: number } | null>(null);
    ...
    const handleOptimizeRoutes = () => {
        ...
        setTimeout(() => {
            setOptimizing(false);
            setOptimizationData({ savings: '18%', routes: 12 });
            setToast({ message: "Routes optimized successfully. 18% fuel savings projected.", type: 'success' });
        }, 2000);
    };
                    ...
                    <TacticalCard title="Fleet Status" className="h-96">
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-center justify-between gap-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold text-blue-700">
                                            {optimizationData ? `Optimization Active. ${optimizationData.savings} savings.` : 'Route optimization recommended.'}
                                        </p>
                                        {optimizationData && <p className="text-[10px] text-blue-500">{optimizationData.routes} routes updated.</p>}
                                    </div>
                                </div>
                                <button
                                ...
                                >
                                    {optimizing ? <RefreshCw size={14} className="animate-spin" /> : <Route size={14} />}
                                </button>
                            </div>
                        </div>
                    </TacticalCard>
