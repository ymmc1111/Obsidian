...
- **Frontend:** Implement CRUD UI in `PlanningView`  
  🚧 Add modal form, edit/delete buttons, validation, and toast notifications.
...
- **Backend:** Add Inventory CRUD endpoints  
  ✅ `addInventoryItem()`, `updateInventoryItem()` are functional on Mock DB.
- **Frontend:** Implement CRUD UI in `InventoryView`  
  ✅ Modal form for Add/Edit, including status and location updates, is implemented.
- **Migrate to Firestore (Optional)**  
  📋 Move inventory data from mock DB to Firestore for real‑time updates and history tracking.
...
- **Shop Floor:** Enhanced Sign‑Off  
  🚧 Persistence (Audit Log) complete. Needs file/photo attachments and Pause/Resume.
- **Traceability:** Precision Recall  
  🚧 Initiate Recall implemented on backend. Needs full UI confirmation and history tracking.
- **Finance:** Invoice Approval Workflow  
  🚧 Approve/Flag implemented on backend. Needs comprehensive role‑check UI.
- **Admin:** User Role Management  
  🚧 User Directory visible. Needs Role Edit/Lock/Unlock functionality.
